...
class DeltaStraddleBot:
    def __init__(self):
        ...
        # Delta Exchange client (using correct version 1.0.13 syntax)
...
Last Update: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
    ...
    def get_same_day_expiry_options(self, atm_strike: int) -> Tuple[Optional[str], Optional[str]]:
        ...
        # Delta Exchange option symbol format: C-BTC-strike-ddmmyy or P-BTC-strike-ddmmyy
        ...
        try:
            ...
            logger.error(f"Error verifying options: {e}")
...
⚠️ **Risk:** Unlimited beyond premium received
    ...
    async def execute_straddle_strategy(self, atm_strike: int, chat_id: int):
        ...
        try:
            ...
            # Execute CE sell order (Correct syntax for delta-rest-client 1.0.13)
            try:
            ...
            # Place stop-loss orders (using bracket order or separate stop orders)
            stop_loss_orders = []
            
            # CE stop-loss (buy back at higher price)
            try:
                ce_stop_order = self.delta_client.place_order(
                    product_id=premium_data['ce_product_id'],
                    side='buy',
                    size=self.lot_size,
                    limit_price=str(premium_data['ce_stop_loss']),
                    order_type='stop_order',
                    stop_price=str(premium_data['ce_stop_loss'] * 0.995)  # Trigger slightly below limit
                )
                ...
                # Try with stop-limit order if stop_order fails
                try:
                    ce_stop_order = self.delta_client.place_order(
                        product_id=premium_data['ce_product_id'],
                        side='buy',
                    ...
                    stop_loss_orders.append(('CE_STOP_MANUAL', ce_stop_order))
                    logger.info(f"CE manual stop-loss placed: {ce_stop_order}")
                except Exception as e2:
                    logger.error(f"Error placing CE manual stop-loss: {e2}")
            
            # PE stop-loss (buy back at higher price)
            try:
                pe_stop_order = self.delta_client.place_order(
                    product_id=premium_data['pe_product_id'],
                    side='buy',
                    size=self.lot_size,
                    limit_price=str(premium_data['pe_stop_loss']),
                    order_type='stop_order',
                    stop_price=str(premium_data['pe_stop_loss'] * 0.995)  # Trigger slightly below limit
                )
                ...
                try:
                    ...
                    stop_loss_orders.append(('PE_STOP_MANUAL', pe_stop_order))
                    logger.info(f"PE manual stop-loss placed: {pe_stop_order}")
                except Exception as e2:
                    logger.error(f"Error placing PE manual stop-loss: {e2}")
...
**Stop-Loss Orders:**
CE Stop: {len([x for x in stop_loss_orders if 'CE_STOP' in x[0]])} placed
PE Stop: {len([x for x in stop_loss_orders if 'PE_STOP' in x[0]])} placed
...
# Run the Flask app for Render.com
if __name__ == "__main__":
    ...
    if webhook_url:
        ...
        try:
            # Set webhook URL
            import asyncio
            asyncio.run(straddle_bot.application.bot.set_webhook(f"{webhook_url}/webhook"))
