...
class DeltaExchangeClient:
    ...
    def get_wallet_balance(self) -> Dict:
        """Get wallet balance and portfolio information"""
        return self.make_request('GET', '/v2/wallet/balances')
    
    def get_positions(self) -> Dict:
        """Get all positions - corrected endpoint"""
        return self.make_request('GET', '/v2/positions')
    
    def get_position_by_product(self, product_id: int) -> Dict:
    ...
    def get_order_history(self) -> Dict:
        """Get order history"""
        return self.make_request('GET', '/v2/orders/history')
    
    def get_fills(self) -> Dict:
        """Get fill/trade history"""
        return self.make_request('GET', '/v2/fills')
    
    def get_portfolio_summary(self) -> Dict:
        """Get comprehensive portfolio summary"""
        try:
            # Get wallet balances
            wallet_response = self.get_wallet_balance()
            ...
            summary = {
                'success': True,
                'wallet': wallet_response.get('result', []) if wallet_response.get('success') else [],
                'positions': positions_response.get('result', []) if positions_response.get('success') else [],
                'open_orders': orders_response.get('result', []) if orders_response.get('success') else [],
                'errors': []
            }
            
            # Track any errors
            if not wallet_response.get('success'):
                summary['errors'].append(f"Wallet: {wallet_response.get('error', 'Unknown error')}")
            if not positions_response.get('success'):
                summary['errors'].append(f"Positions: {positions_response.get('error', 'Unknown error')}")
            if not orders_response.get('success'):
                summary['errors'].append(f"Orders: {orders_response.get('error', 'Unknown error')}")
            
            return summary
            
        except Exception as e:
            return {'success': False, 'error': f'Portfolio summary failed: {str(e)}'}
    ...
    def cancel_all_orders(self) -> Dict:
        ...
        return self.make_request('DELETE', '/v2/orders/all')
